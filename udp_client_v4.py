"""@package udp_client_v4
A module to generate UDP/IPv4 packets over a Ethernet network interface.

USAGE: python udp_client_v4.py <destination ip> <udp port> <packet gap> <message size>.
...
# INPUT ARGS
num_args_exp = 5
usage = "usage: python " + sys.argv[0] + " <destination ip> <udp port> <packet gap> <message size>"
if len(sys.argv) != num_args_exp:
    sys.exit(usage)
else:
    cmd, dst_ip, udp_port, packet_gap, message_size = sys.argv
